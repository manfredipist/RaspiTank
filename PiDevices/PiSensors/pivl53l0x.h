...
class PiVL53L0X : public QObject
{
    Q_OBJECT
public:
    explicit PiVL53L0X(uint8_t i2c_bus, uint8_t i2c_address, QObject *parent = nullptr);
    ~PiVL53L0X();
public slots:
    void initialize();
    double getDistance();
private slots:
    ...
    void performVhvCalibration();
    void performPhaseCalibration();
    void performRefSpadManagement();

    void performSingleRefCalibration(uint8_t byte);
    void refCalibrationIo(uint8_t byte);

    void performRefSignalMeasurement();
    void performSingleRangingMeasurement();

    void performSingleMeasurement();
    void getRangingMeasurementData();
